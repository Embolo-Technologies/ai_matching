...
class FastMatcher:
    ...
    def validate_match(self, query: str, candidate: dict, name: str = "", compname: str = "", strict: bool = False) -> bool:
        ...
        # Company mismatch check (strict mode only — for AI guardrail)

        if strict and compname and compname.strip() and compname.strip() != '--':

            c_brand_company = self.normalize_text(candidate.get('brand', '') or '')

            q_comp_norm = self.normalize_text(compname)

            if q_comp_norm and c_brand_company:

                comp_sim = fuzz.partial_ratio(q_comp_norm, c_brand_company)
